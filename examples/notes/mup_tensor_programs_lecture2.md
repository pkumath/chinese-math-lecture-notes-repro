...
本讲的数学核心是一个可算表。对宽度为 `$`n`$`、base 宽度为 `$`n_0`$` 的普通 MLP，隐藏层初始化通常仍是 fan-in 标准差；但输出层 `MuReadout` 的标准差要再除以 `$`\sqrt{n/n_0}`$`。因此当 hidden fan-in 是 `$`n`$` 时，标准参数化的 readout 标准差是 `$`n^{-1/2}`$`，而 μP target readout 的标准差是 `$`\sqrt{n_0}/n`$`。若取 `$`n_0=1`$`，就是常见的 `$`1/n`$` readout 尺度。
...
<tr><td>readout 怎么初始化</td><td>比标准最后一层小一档</td><td>`MuReadout`</td><td>target 宽度 `$`n`$` 时 std 乘 `$`\sqrt{n_0/n}`$`</td></tr>
...
**例子。** base MLP hidden width 是 256，target hidden width 是 1024，则 `$`\widetilde n=4`$`。如果某个 readout 的标准初始化标准差是 `$`1/\sqrt{1024}`$`，μP target readout 还要再除以 `$`\sqrt 4`$`，得到 `$`1/64`$`。
...
<tr><td>Output weight</td><td>infinite</td><td>finite</td><td>最后层 `Linear(n, d_out)`</td><td>比标准 fan-in 初始化再除以 `$`\sqrt{\widetilde n}`$`</td></tr>
</table>
...
如果 base width 取 `$`n_0=1`$`，这就是 `$`1/n`$` readout 尺度；如果 base width 是 256，则 target 的 readout 不是绝对 `$`1/n`$`，而是相对 base 标准层缩小 `$`\sqrt{256/n}`$`。

### 定理 2.2：三层 MLP 的 μP 初始化保持 hidden 坐标有限，并使初始 readout 小一档

考虑固定输入 `$`x\in\mathbb R^d`$`，宽度 `$`n`$` 的三层 MLP：
...
特别地，若 base width `$`n_0`$` 固定而 target width `$`n\to\infty`$`，初始输出趋向零，但 hidden preactivation 坐标不退化。
...
因此 `$`h^1_i=O_p(1)`$`。令 `$`q_1=\mathbb E\phi(G_1)^2`$`，其中 `$`G_1`$` 是第一层极限 Gaussian；在本假设下 `$`q_1<\infty`$`。条件在第一层激活上，第二层满足
...
同样由坐标自平均，括号内保持 `$`O_p(1)`$`，所以 `$`\operatorname{Var}(f)=O(n_0/n)`$`。这完成证明。

### 假设在哪里用

- 第一层 fan-in 初始化用来让输入层 preactivation 不随 hidden width 变动。
- 第二层 `$`1/n`$` 方差用来抵消 `$`n`$` 个 hidden 输入的求和。
- 输出层 `$`n_0/n^2`$` 方差是 μP 特有的小一档 readout；它使 readout 在 target 宽度下比 SP 小 `$`\sqrt{n/n_0}`$` 倍。
- 激活二阶矩有限用来控制每层经验平方平均。

### Worked example：base 256 到 target 4096

设 base width 是 256，target width 是 4096。则 `$`\widetilde n=16`$`。若普通最后一层 `Linear(4096, d_out)` 的 Xavier/LeCun 标准差按 `$`1/\sqrt{4096}=1/64`$` 量级，那么 μP readout 标准差应再除以 4，得到 `$`1/256`$` 量级。隐藏层 `Linear(4096,4096)` 仍是 fan-in 标准差 `$`1/64`$` 量级。

### 非例子：把所有层都用标准 fan-in 初始化

若最后一层也用 `$`1/\sqrt n`$` 标准差，则初始输出方差是 `$`O(1)`$`，但第一步最大更新分析会把 readout 更新和 hidden 更新放在不兼容的尺度上。Tensor Programs V 的 μP 表正是通过让 output weights 小一档、再配套改 input/output/bias 学习率来恢复可迁移的宽度极限。
...
考虑上一节的三层 MLP，并用单样本平方损失 `$`\mathcal L=(f-y)^2/2`$` 做一阶梯度下降。假设 `$`f-y=O_p(1)`$`，`$`\phi`$` 和 `$`\phi'`$` 在相关坐标上有有界矩。对输出权重使用学习率
...
1. 输出权重梯度是 `$`(f-y)\phi(h^2_j)`$`，单坐标量级为 `$`O_p(1)`$`。
...
括号内由坐标自平均保持 `$`O_p(1)`$`，而 `$`f-y=O_p(1)`$`，所以 `$`\Delta f_{\rm out}=O_p(1)`$`。这说明小 readout 初始化不会阻止输出在训练第一步发生有限变化；它只是让初始化和训练更新处于 μP 的可迁移坐标尺度。

### 命题 3.2：hidden 层更新为什么也能是有限特征移动

在同一模型中，第二层 hidden 权重 `$`W^2_{ji}`$` 的梯度含有一个 readout 权重因子：
...
由于 μP 初始化下 `$`W^3_j=O_p(\sqrt{n_0}/n)`$`，单个 hidden 权重的梯度很小。但 preactivation 更新是对 `$`i`$` 求和：
...
若 hidden 权重使用 `$`O(1)`$` master 学习率，则每项 `$`\Delta W^2_{ji}`$` 是 `$`O_p(1/n)`$` 量级，求和后得到 `$`O_p(1)`$` 的 preactivation 更新。这个“单权重小、坐标向量更新大”的现象正是 Tensor Programs V 中 outer-product 梯度表的含义。
...
如果输出权重和输入 embedding 共享，应使用 `MuSharedReadout`。如果是 Transformer attention，μP 常要求 attention logits 用 `$`1/d`$` 型缩放，而不是普通 `$`1/\sqrt d`$` 型缩放；官方示例为了兼容 head dimension 64，写成类似 `8 / d` 的常数约定。
...
给定一组宽度 `$`n_1<n_2<\cdots<n_K`$`，对每个宽度构造同一 μP family 的模型。对每个感兴趣的 activation 向量 `$`z^{(n)}_\ell`$`，记录

$$
C_{\ell,t}(n)=\frac{1}{\dim z^{(n)}_\ell}\sum_i |z^{(n)}_{\ell,t,i}|,
$$

其中 `$`t`$` 是训练步数。若对固定 `$`\ell,t`$`，`$`C_{\ell,t}(n)`$` 随 `$`n`$` 大致水平，则该坐标通过检查。若随宽度爆炸或趋零，就说明某个初始化、学习率或 shape 标记错了。

### 命题 5.2：coord check 是第 1 讲坐标经验律的工程版本

如果一个 μP family 在有限训练步内有 Tensor Program 极限，并且某层坐标经验律收敛到非退化随机变量 `$`Z_{\ell,t}`$`，那么在一阶矩可积条件下
...
这是第 1 讲坐标经验律定义的直接应用。取测试函数 `$`\psi(z)=|z|`$`。若经验律收敛并且一阶矩一致可积，则
...
- 初始化时 output 或 attention logits 可在 `$`1/\sqrt n`$` 量级下沉；官方建议可用 zero readout 或 query-zero 初始化去消除初始差异，但训练几步后曲线应变平。
...
1. 解释 width multiplier `$`\widetilde n=n/n_0`$` 的含义。
...
1. base width 为 512，target width 为 8192。若普通 readout std 是 `$`1/\sqrt{8192}`$`，μP readout std 应缩小多少倍？
...
2. 假设 readout 仍用 SP std `$`1/\sqrt n`$`，但 optimizer 用 μP 学习率。计算初始输出方差和 readout 首步贡献，说明哪里和 μP 目标冲突。
