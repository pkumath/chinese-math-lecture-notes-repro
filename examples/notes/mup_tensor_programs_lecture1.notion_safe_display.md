...
3. 在本讲固定的 readout-average convention 下，输出写成 

$$
n^{-1}\sum_i a_i\phi(w_i\cdot x)
$$
...
$$
 ...
 时，每个 neuron 的坐标更新有非零有限极限，输出仍保持 `$`O(1)`$`；这就是 toy μP 的最大有限更新原则。
...
**非例子。** 若 `$`u_i=Z`$` 对所有 `$`i`$` 共用同一个随机变量，则 

$$
n^{-1}\sum_i\psi(u_i)=\psi(Z)
$$

 不收敛到非随机常数；坐标之间没有足够的平均化。Tensor Program master theorem 的条件本质上是在防止这种全坐标共享噪声破坏自平均。
...
并且网络输出在有限输入集上也保持 `$`O(1)`$`，则称这个坐标处在最大有限更新尺度。若 
...
$$
W_n\in\mathbb R^{n\times n}
$$

 的元素独立且服从 `$`N(0,1/n)`$`。给定 `$`k`$` 个随机或确定性向量 

$$
x_n^1,\ldots,x_n^k\in\mathbb R^n
$$

，假设它们与 `$`W_n`$` 独立，并且经验 Gram 矩阵逐项依概率收敛：
...
$$
(z_{n,i}^1,\ldots,z_{n,i}^k)
$$

 是 Gaussian，协方差正是 `$`Q_n`$`。
...
$$
\mathcal F_n=\sigma(x_n^1,\ldots,x_n^k)
$$
...
同一行 `$`i`$` 给出一个 `$`k`$` 维 Gaussian 向量；不同行使用互相独立的权重，所以这些 `$`k`$` 维向量在条件下独立同分布。令 

$$
G_n\mid\mathcal F_n\sim N(0,Q_n)
$$
...
$$
 ...
 依概率，Gaussian 向量 `$`N(0,Q_n)`$` 的分布依概率趋于 `$`N(0,Q)`$`。对有界连续 
...
$$
 ...
 的结论。还需证明截断误差可以一致控制。由 Gaussian 矩估计，若 

$$
\|Q_n\|_{op}
$$

 在概率意义下有界，则对任意固定 `$`r`$`，随机变量 

$$
\mathbb E[\|G_n\|^r\mid\mathcal F_n]
$$
...
可由更高阶 Gaussian 矩和 Markov 不等式控制到任意小，概率意义下一致成立。经验平均侧同理：条件在 

$$
\mathcal F_n
$$

 上使用同一个尾部期望界，再用 Markov 不等式控制。于是先令 
...
$$
x^1,\ldots,x^m\in\mathbb R^d
$$

。考虑宽度为 `$`n`$` 的 `$`L`$` 层 MLP。第一层为
...
$$
W^1_{ir}\sim N(0,\sigma_w^2/d)
$$

，

$$
b_i^1\sim N(0,\sigma_b^2)
$$
...
$$
 ...
 层条件使用定理 2.1，再加上 bias 的独立 Gaussian 项。
...
$$
n^{-1}\sum_j y_j^a y_j^b
$$

 收敛到 

$$
\sigma_w^2\mathbb E[\phi(G_a^{\ell-1})\phi(G_b^{\ell-1})]
$$
...
等价于对向量 `$`y^a`$` 施加定理 2.1。它给出一个条件 Gaussian 坐标族，其极限协方差是 
...
$$
 ...
 到每个 `$`(a,b)`$` 位置，因为同一个 neuron 的 bias 对所有输入共享。因此极限协方差正是 
...
，定理 2.1 的截断论证逐层适用；多项式增长经过 Gaussian 矩控制仍然有界于有限阶矩。归纳完成。
...
$$
 ...
 缩放使 preactivation 方差保持 `$`O(1)`$`。
- 层间独立使条件化后可以调用定理 2.1。
- bias 对输入共享，因此给所有 `$`K_{ab}`$` 加同一个 
...
，极限通常是随机的而不是确定 kernel。这个非例子再次说明：坐标自平均假设不是装饰。
...
本节只研究一个两层模型，但会完整证明缩放临界性。设训练集 `$`(x_s,y_s)_{s=1}^M`$` 固定，
...
其中 `$`a_i,w_i`$` 初始化为 `$`O(1)`$` 坐标，使用宽度无关学习率 

$$
\eta
$$

 做一步梯度下降。若训练误差 `$`e_s=f_n(x_s)-y_s`$` 在概率意义下有界，则每个固定 neuron 的隐藏权重更新满足
...
$$
\Delta w_i=-\eta\partial\mathcal L/\partial w_i
$$
...
。在训练点 `$`x_t`$` 上，readout 更新贡献的一阶项为

$$
\frac1{\sqrt n}\sum_i \Delta a_i\phi(w_i\cdot x_t),
$$

这是 `$`n`$` 个量级 `$`1/n`$` 的相关平均，总量一般为 `$`O_p(1)`$`。更具体地，把 
...
。对 `$`a_i`$` 和 `$`w_i`$` 同时做一步梯度下降。若 

$$
\lambda_n/n\to\eta\in(0,\infty)
$$

，则每个 neuron 的更新有非零有限极限尺度：
...
$$
 ...
 且对应梯度分量在某个正概率事件上不为零，则坐标更新不紧。这说明 

$$
\lambda_n\asymp n
$$

 是本 convention 下的最大有限更新尺度。
...
，并且存在某个 `$`c>0`$` 与正概率事件 `$`E`$`，使括号内梯度向量范数至少为 `$`c`$`，则在 `$`E`$` 上更新范数至少为 
...
$$
\rho_n=n^{-1}\sum_i\delta_{(a_i,w_i)}
$$

 是 neuron 经验测度。一步更新可以写成坐标映射
...
$$
n^{-1/2}\sum_i a_i\phi(w_i\cdot x)
$$
...
，通常爆炸。这说明 μP 不是“学习率越大越好”；它必须和输出缩放、初始化缩放一起设计。

**本节带走什么。**

- NTK 缩放能训练输出，但隐藏特征一步移动趋零。
- readout-average convention 中 
...
$$
Q_n^{ab}=n^{-1}\sum_jx_j^ax_j^b
$$

？
3. 说明 NNGP 递推中 bias 为什么给所有 `$`(a,b)`$` 位置加 
...
$$
f_n(x)=n^{-\alpha}\sum_i a_i\phi(w_i\cdot x)
$$
...
$$
 ...
 增长快于任意多项式，说明定理 3.1 的矩控制步骤会卡在哪里。
...
本讲已经完整证明了三个基础事实。第一，随机矩阵乘法把经验 Gram 矩阵转成 Gaussian 坐标语义。第二，MLP 的 NNGP kernel 递推来自逐层应用这个语义。第三，在最小两层模型里，NTK 缩放导致隐藏特征懒惰，而 readout-average convention 下 
