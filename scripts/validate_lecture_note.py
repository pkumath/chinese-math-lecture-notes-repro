#!/usr/bin/env python3
"""Validate Chinese math lecture-note skill files and draft notes.
...
SUMMARY_HEADING_PATTERN = re.compile(r"^##\s*一页摘要\s*$", re.M)

FRONT_ROADMAP_HEADING_PATTERN = re.compile(
    r"^##+\s*(?:前置路线图|路线图|学习路径图|依赖图|路线图与关系图)\b",
    re.M,
)

CORE_EXAMPLE_HEADING_PATTERN = re.compile(
    r"^##+\s*(?:\d+[.、]\s*)?(?:贯穿例子|核心例子|主例子|从一个模型开始)\b",
    re.M,
)
REPRODUCTION_HEADING_PATTERN = re.compile(
...
TERMINOLOGY_HEADING_PATTERN = re.compile(
...
TOC_HEADING_PATTERN = re.compile(r"^##\s*目录\s*$", re.M)
...
def check_skill(skill_dir: Path) -> None:
    ...
    if "Proof-Granularity Gate" not in text:
        fail("SKILL.md must include the proof-granularity gate")
    if "Standard Sync Gate" not in text:
        fail("SKILL.md must include the standard-sync gate")
    if "One-Page Summary Gate" not in text:
        fail("SKILL.md must include the one-page summary gate")
    if "Front Roadmap Gate" not in text:
        fail("SKILL.md must include the front-roadmap gate")
    if "Learning-Load and Reproduction Gate" not in text:
        fail("SKILL.md must include the learning-load and reproduction gate")
    if "Source-Spine Coverage Gate" not in text:
        fail("SKILL.md must include the source-spine coverage gate")
    ...
    if "逐节证明颗粒度" not in rubric:
        fail("quality rubric must include section-by-section proof granularity")
    if "一页摘要门槛" not in rubric:
        fail("quality rubric must include the one-page summary gate")
    if "中文优先术语门槛" not in rubric:
        fail("quality rubric must include the Chinese-first terminology gate")
    if "前置路线图" not in rubric:
        fail("quality rubric must include the front-roadmap gate")
    if "复现测试" not in rubric or "贯穿例子" not in rubric:
        fail("quality rubric must include the learning-load/reproduction gate")
    if "主题主干" not in rubric:
        fail("quality rubric must include the source/request spine coverage gate")
    if "<table_of_contents" not in rubric:
        fail("quality rubric must include the clickable Notion table of contents rule")
    print("OK: skill structure")
...
def check_chinese_terminology(text: str, mode: str) -> None:
    if mode != "full":
        return
    ...
    for match in ENGLISH_WORD_PATTERN.finditer(scan_text):
        ...
        if word.isupper() and 3 <= len(word) <= 8:
            continue
        words.append(lower)
    if not TERMINOLOGY_HEADING_PATTERN.search(text):
...
def check_learning_load_and_reproduction(text: str) -> None:
    core_match = CORE_EXAMPLE_HEADING_PATTERN.search(text)
    if not core_match:
    ...
    theorem_match = THEOREM_HEADING_PATTERN.search(text)
    ...
    repro_match = REPRODUCTION_HEADING_PATTERN.search(text)
    if not repro_match:
    ...
    for label, pattern in [
        ...
        if not re.search(pattern, repro_block):
            fail(f"reproduction test is missing {label} criteria")
    core_phrase_count = len(re.findall(r"贯穿例子|核心例子|主例子", text))
...
def check_one_page_summary(text: str) -> re.Match[str]:
    summary_match = SUMMARY_HEADING_PATTERN.search(text)
    if not summary_match:
        fail("missing `## 一页摘要` section")
    next_heading = re.search(r"^##\s+", text[summary_match.end():], flags=re.M)
    if not next_heading:
        fail("`## 一页摘要` must be followed by another section heading")
    ...
    missing = [name for name, pattern in required_groups if not re.search(pattern, plain_summary)]
    if missing:
        fail("one-page summary misses orientation signals: " + ", ".join(missing))
    return summary_match


def check_note(note: Path, mode: str, required_terms: list[str] | None = None) -> None:
    ...
    check_chinese_terminology(text, mode)
    if mode == "full":
        summary_match = check_one_page_summary(text)
        ...
        first_substantive = re.search(r"^##\s+(?:预备知识|读者画像|\d+\.|一、)", text, flags=re.M)
        ...
        roadmap_match = FRONT_ROADMAP_HEADING_PATTERN.search(text)
        ...
        if not any(re.search(p, roadmap_block, flags=re.M) for p in VISUAL_PATTERNS):
            fail("front roadmap must contain a visual path/dependency block, not only prose")
        check_learning_load_and_reproduction(text)
    ...
    missing = [p for p in REQUIRED_NOTE_PATTERNS if not re.search(p, text)]
    if missing and mode == "full":
        fail("missing required lecture sections/patterns: " + ", ".join(missing))
    if mode == "full" and not any(re.search(p, text, flags=re.M) for p in VISUAL_PATTERNS):
    ...
    if mode == "full":
        ...
        theorem_count = len(re.findall(r"定理|命题|引理", text))
        if theorem_count and not re.search(r"证明路线|证明脊柱|proof spine|proof route", text, flags=re.I):
            fail("theorem-like content exists but no proof spine/route is marked")
        if theorem_count and "假设在哪里用" not in text:
            fail("theorem-like content exists but hypotheses-use explanation is missing")
        major_sections = len(
            re.findall(r"^##\s+(?:\d+\.|[一二三四五六七八九十]+|总结|下一步|Reference)", text, flags=re.M)
        )
        takeaways = len(re.findall(r"本(?:节|小节)带走什么", text))
...
def main() -> None:
    ...
    if args.note:
        check_note(args.note, args.mode, args.required_term)
